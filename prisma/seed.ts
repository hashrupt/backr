...
async function main() {
  ...
  const unclaimedApp = await prisma.entity.upsert({
  ...
  const superValidator = await prisma.entity.upsert({
  ...
  console.log("Created entities:", {
    coolApp: coolApp.id,
    nodeRunner: nodeRunner.id,
    unclaimedApp: unclaimedApp.id,
    superValidator: superValidator.id
  });
  ...
  console.log("Created campaigns:", {
    ...
    campaign3: campaign3.id
  });

  // Create backings for campaign1
  await prisma.backing.upsert({
    where: {
      userId_entityId_campaignId: {
        userId: bob.id,
        entityId: coolApp.id,
  ...
  // Create an accepted interest (ready to complete)
  await prisma.interest.upsert({
    where: {
      userId_campaignId: {
        userId: dave.id,
